...
2. **Match the project's commit style.** Read the last 10 commit messages with `git log --oneline -10` and write a message that matches the tone, format, and conventions used.

3. **One commit.** Stage all session changes and commit them together.

## Flow

1. Run `git status` and `git diff` to identify changes made in this session.
